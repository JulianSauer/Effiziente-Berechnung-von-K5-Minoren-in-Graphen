...
Als Eingabe wird ein ungerichteter Graph ohne Mehrfachkanten oder Schleifen erwartet, ausgegeben wird, ob ein $K_5$-Minor enthalten ist oder nicht.
...
\begin{theorem}\label{eq:Theorem33}
  ...
  Jede durch $C$ definierte augmentierte Komponente ist ein Minor von $G$ \gdw es entweder mindestens $k$ Komponenten sind oder mindestens zwei der Komponenten jeweils aus mehr als einem Knoten bestehen.
\end{theorem}
\begin{beweis}
...
%  Sei $G$ ein $3$-zusammenhängender Graph mit einem $W$-Subdivision.
...
\begin{beweis}
  ...
  Dann ist $G$ ein Subdivision zu $W$ und enthält laut Lemma \ref{eq:Theorem36} keinen \kf-Minor bei Isomorphie zu $W$.
\end{beweis}
...
Kezdy und McGuinness verwenden den Williamson-Algoritmus,\cite{Wil84} welcher in Linearzeit für einen Graph einen \kf- \bzw \kdd-Subdivision ausgibt oder feststellt, dass der Graph planar ist.
...
\begin{enumerate}
  \item \label{item:algorithmus11} Solange es $1$-Separatoren gibt: Erzeuge augmentierte Komponenten mit diesen Separatoren.
  \item \label{item:algorithmus12} Solange es $2$-Separatoren gibt: Erzeuge augmentierte Komponenten mit diesen Separatoren.
\end{enumerate}
Für alle $j$ Zusammenhangskomponenten $Z_i$ von $G$:
\begin{enumerate}[resume]
  \item \label{item:algorithmus13} Planaritätstest:
                                   \begin{enumerate}
                                     \item $Z_i$ ist planar: Falls $i<j$ behandle $Z_{i+1}$ in Schritt \ref{item:algorithmus13}, sonst stopp.
                                     \item $Z_i$ enthält ein \kf-Subdivision: Stopp
                                     \item $Z_i$ enthält ein \kdd-Subdivision: Weiter zu Schritt \ref{item:algorithmus14}
                                   \end{enumerate}
  \item \label{item:algorithmus14} $Z_i$ ist isomorph zu $W$: Falls $i<j$ behandle $Z_{i+1}$ in Schritt \ref{item:algorithmus13}, sonst stopp.
                                   $Z_i$ ist nicht isomorph zu $W$: Weiter zu Schritt \ref{item:algorithmus15}
  \item \label{item:algorithmus15} Seien $R = \{a, b, c\}$ die roten und $B = \{x, y, z\}$ die blauen Branch-Ends des \kdd-Subdivision.
                                   Bildet $R$ einen $(3, 3)$-Separator, sodass alle Knoten aus $B$ in unterschiedlichen Zusammenhangskomponenten sind?
                                   Dann erzeuge die augmentierten Komponenten mit $R$ als Separator und gehe zu Schritt \ref{item:algorithmus13}.
                                   (Analog für $B$ als Separator)
  \item \label{item:algorithmus16} $Z_i$ enthält nach Theorem \ref{eq:Theorem36} einen \kf-Minor: Stopp.
\end{enumerate}
